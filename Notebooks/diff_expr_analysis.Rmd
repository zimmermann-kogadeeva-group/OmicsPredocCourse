...
df_go_terms <- df_lfc %>% 
filter(padj < 0.05 & abs(log2fc) > 1) %>%
group_by(comparison, strain) %>%
group_map(~{
    .x %>%
    pull(gene_id) %>%
    paste0(collapse=",") %>%
    get_ora() %>%
    mutate(comparison = .y$comparison, strain = .y$strain)
}) %>%
bind_rows() %>%
mutate(term=paste0(label, " (", id, ")"))
```

```{r}
df_go_terms %>%
filter(strain == "090.3", comparison == "CvsP", fdr < 0.01) %>%
arrange(fdr) %>%
ggplot(aes(y=term, x=number_in_list, fill=fdr)) + 
